...
module.exports = class ParserImpl extends Parser 
{
  ...
  createParserCommentList(debug) 
  {
    ...
    for (let c = 0; c < multiLineComments.length; c++) {
      ...
      const commentStartLine = this.fileContent.substr(0, current.index).split(split_new_line).length - 1;
      const commentEndLine = commentStartLine + comment.split(split_new_line).length - 1;
      const nextLineStartIndex = current.index + current[0].length + 1;
      const nextLine = this.fileContent.substr(nextLineStartIndex).split(split_new_line)[0];
